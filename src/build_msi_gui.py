...
class BuilderGUI:
    ...
    def generate_setup_py(self):
        """Generate cx_freeze_setup.py file for the project"""
        ...
        # Determine if we need to include src modules
        # Check if main script is in src directory
        main_script_dir = os.path.dirname(self.main_script.get())
        if os.path.basename(main_script_dir) == "src":
            # Include src as a package
            packages_to_include.append("src")
...
'''
        ...
        # Get relative path from project directory to main script
        project_dir = self.project_path.get()
        main_script = self.main_script.get()
        try:
            # Calculate relative path
            main_script_rel = os.path.relpath(main_script, project_dir)
        except ValueError:
...
executables = [
    Executable(
        "{main_script_rel.replace(os.sep, '/')}",
        base=base,
...
'''
        ...
        setup_path = os.path.join(self.project_path.get(), "cx_freeze_setup.py")
        try:
            with open(setup_path, 'w', encoding='utf-8') as f:
                f.write(setup_content)
            self.log(f"✓ Đã tạo file cx_freeze_setup.py")
            return True
        except Exception as e:
            self.log(f"❌ Lỗi khi tạo cx_freeze_setup.py: {e}")
            return False
    ...
    def run_build_command(self, command, description):
        ...
        # Generate cx_freeze_setup.py
        if not self.generate_setup_py():
            messagebox.showerror("Lỗi", "Không thể tạo file cx_freeze_setup.py!")
            return
        ...
        def build_thread():
            try:
                ...
                if uv_available:
                    cmd = ["uv", "run", "python", "cx_freeze_setup.py"] + command
                else:
                    cmd = [sys.executable, "cx_freeze_setup.py"] + command
